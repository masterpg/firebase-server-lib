...
program
  ...
  .action(async (id_or_path: string, cmdObj: { format: OutputFormat; json?: boolean }) => {
    ...
    if (nodeType === 'Dir') {
      ...
      const { list } = await storageService.getDescendants({ path: nodePath, includeBase: true }, { size: count })
...
program
  ...
  .action(async (id_or_path: string, cmdObj: { format: OutputFormat; json?: boolean }) => {
    ...
    if (nodeType === 'Dir') {
      ...
      const { list } = await storageService.getChildren({ path: nodePath, includeBase: true }, { size: count })
      StorageService.sortNodes(list)
      print([...ancestors, ...list], cmdObj.format, nodePath)
    }
    // 取得されたノードがファイルの場合
